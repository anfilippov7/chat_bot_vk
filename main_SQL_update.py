...
def people_record_data(data: json) -> 'record SQL':
    ...
    try:
        q_people = session.query(People).filter(
            People.auser_id == auser_id)  # Делаем запрос в базу данных People у конкретного auser_id
        id_list = [record_id.search_vk_id for record_id in
                   q_people.all()]  # Делаем list comprehensions с существующими id в базе данных People у конкретного auser_id
        for record in data:
            if record[
...
def blacklist_record_data(auser_id, id_blacklist: int) -> 'record blacklist SQL':
    ...
    try:
        ...
        q_blacklist = session.query(Blacklist).filter(Blacklist.auser_id == auser_id).filter(
            Blacklist.search_vk_id == id_blacklist)  # Делаем запрос в базу данных Blacklist
        id_list_blacklist = [record_id.search_vk_id for record_id in q_blacklist.all()]  # Делаем list comprehensions с существующими id в базе данных Blacklist
...
def favourites_record_data(auser_id, id_favourite: int) -> 'record favourite SQL':
    ...
    try:
        ...
        q_favourite = session.query(Favourite).filter(Favourite.auser_id == auser_id).filter(
            Favourite.search_vk_id == id_favourite)  # Делаем запрос в базу данных Favourite
...
def display_data_people(auser_id: int):
    ...
    for search in q_people.all():
        if search.search_vk_id not in id_list_blacklist:
            print(search.id, search.auser_id, search.search_vk_id, search.first_name, search.last_name,
                  search.profile_link, search.first_likes, search.second_likes, search.third_likes)
    return True


def display_favorite(auser_id: int):
    q_favorite = session.query(Favourite).filter(
        Favourite.auser_id == auser_id)  # Делаем запрос в базу данных Blacklist
    for search in q_favorite.all():
        print(search.id, search.auser_id, search.search_vk_id, search.first_name, search.last_name, search.profile_link,
              search.first_likes, search.second_likes, search.third_likes)
    return True
